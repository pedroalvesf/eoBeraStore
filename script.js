...
async function getDb() {
    ...
    data = await res.json();
    if (document.querySelector(".container-itens-wish")) {
        let wishListArray = [];
...
function createCard(weaponLink, weaponName, weaponImg, weaponPrice, weaponExterior, weaponStatTrack) {
    ...
    <div class="price">
        <p>${weaponPrice}</p>
        <div class="heart ${wishlist.includes(weaponName.replace(/&#39/g, "'")) ? "choosenSkin" : ""}"> &hearts;</div>
    </div>
...
function selectedSkin(event) {
    let skinChoosedName = event.target.parentNode.parentNode.firstElementChild.firstElementChild.innerText;
